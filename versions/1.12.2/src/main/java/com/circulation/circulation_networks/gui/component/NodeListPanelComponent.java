...
public final class NodeListPanelComponent extends DraggableComponent implements SliderParent {
            ...
            protected @NotNull List<LocalizedComponent> getTooltip(int mouseX, int mouseY) {
                ...
                if (hitSlot >= 0) {
                    return Collections.singletonList(
                        () -> I18n.format("tooltip.circulation_networks.double_click_locate")
                    );
                }
    ...
    private void renderEntry(NodeSnapshotEntry entry, int entryAbsoluteX, int entryAbsoluteY) {
        if (!renderSpriteIfPresent(entryAbsoluteX, entryAbsoluteY)) {
            Gui.drawRect(entryAbsoluteX, entryAbsoluteY, entryAbsoluteX + ENTRY_WIDTH, entryAbsoluteY + ENTRY_HEIGHT, FALLBACK_ENTRY_COLOR);
        }

        renderEntryItem(entry, entryAbsoluteX + ITEM_X, entryAbsoluteY + ITEM_Y);

        Minecraft mc = Minecraft.getMinecraft();
        String coords = mc.fontRenderer.trimStringToWidth(formatCoordinates(entry), COORD_WIDTH);
    ...
    private void renderEntryItem(NodeSnapshotEntry entry, int x, int y) {
        ...
        GlStateManager.translate(x + ITEM_RENDER_SHIFT_X, y + ITEM_RENDER_SHIFT_Y, 150.0F);
        ...
        try {
            ...
            renderItem.renderItemAndEffectIntoGUI(mc.player, stack, 0, 0);
            renderItem.renderItemOverlayIntoGUI(mc.fontRenderer, stack, 0, 0, null);
        } finally {
            renderItem.zLevel = prevZLevel;
        }
    ...
    private boolean renderSpriteIfPresent(int x, int y) {
        if (com.circulation.circulation_networks.gui.component.base.ComponentAtlas.INSTANCE.getRegion(NodeListPanelComponent.ENTRY_SPRITE) == null) {
            return false;
        }
    ...
    private void handleIconDoubleClick(int visibleSlot) {
        ...
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.player != null) {
            NodeHighlightRenderingHandler.INSTANCE.highlight(pos, mc.player.dimension);
            mc.player.sendMessage(new TextComponentString(
                "§e[CFN] §r" + I18n.format("message.circulation_networks.node_location",
