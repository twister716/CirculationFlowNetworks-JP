...
public final class NodeListPanelComponent extends DraggableComponent implements SliderParent {
    ...
    private final Map<String, ItemStack> stackCache = new Object2ObjectOpenHashMap<>();
    ...
    private void renderHeaderCountText(int x, int y) {
        ...
        GuiGraphicsCompat.drawString(guiGraphics, mc.font, text, drawX, y, TEXT_COLOR, false);
    }
    ...
    private void renderEntryItem(NodeSnapshotEntry entry, int x, int y) {
        ...
        GuiGraphicsCompat.translate(guiGraphics, x + ITEM_RENDER_SHIFT_X, y + ITEM_RENDER_SHIFT_Y, 150.0F);
        GuiGraphicsCompat.scale(guiGraphics, ITEM_RENDER_SCALE, ITEM_RENDER_SCALE, 1.0F);
        GuiGraphicsCompat.renderItem(guiGraphics, stack, 0, 0);
        GuiGraphicsCompat.renderItemDecorations(guiGraphics, Minecraft.getInstance().font, stack, 0, 0, getItemCountOverlayText(stack));
    ...
    private ItemStack resolveStack(String itemId) {
        ...
        var itemHolder = BuiltInRegistries.ITEM.get(Identifier.parse(itemId)).orElse(null);
