...
public final class PortNodeRenderer implements BlockEntityRenderer<BlockEntityPortNode, CFNBlockEntityRenderState<BlockEntityPortNode>> {
    ...
    @Override
    public @NonNull CFNBlockEntityRenderState<BlockEntityPortNode> createRenderState() {
        return new CFNBlockEntityRenderState<>();
    }

    @Override
    public void extractRenderState(@NotNull BlockEntityPortNode blockEntity, @NotNull CFNBlockEntityRenderState<BlockEntityPortNode> renderState,
    ...
    @Override
    public void submit(@NotNull CFNBlockEntityRenderState<BlockEntityPortNode> renderState, @NotNull PoseStack poseStack,
                       @NotNull SubmitNodeCollector submitNodeCollector, @NotNull CameraRenderState cameraRenderState) {
        BlockEntityPortNode te = renderState.blockEntity;
        ...
        try (RotatingModelVBORenderer.RenderSession ignored = RotatingModelVBORenderer.beginRenderSession(submitNodeCollector)) {
            ...
            RotatingModelVBORenderer.renderAmbientLit(poseStack, te.getLevel(), te.getBlockPos(), state, PORT_IN_BASE,
            ...
            poseStack.popPose();
            RotatingModelVBORenderer.renderAmbientLit(poseStack, te.getLevel(), te.getBlockPos(), state, PORT_OUT_BASE,
                bottomAngle, CENTER, CENTER, CENTER, 0.0F, 1.0F, 0.0F);
            RotatingModelVBORenderer.renderFullBrightYAxis(poseStack, state, PORT_OUT_EMISSIVE,
