...
public final class API {
     ...
     * Note: the returned set may include tile entities that also carry an {@code IMachineNode}.
     *
    ...
    /**
     * 判断指定 TileEntity 是否位于能源全局黑名单中。
     * 黑名单中的实体不会被任何节点识别为能源容器。
     * <p>
     * Returns whether the given TileEntity is on the global energy blacklist.
     * Blacklisted entities are never recognized as energy containers by any node.
     *
     * @param tileEntity 目标实体 / the tile entity to check
     * @return 是否在黑名单中 / {@code true} if blacklisted
     */
    public static boolean isEnergyBlacklisted(@Nonnull TileEntity tileEntity) {
        return RegistryEnergyHandler.isBlack(tileEntity);
    }

    /**
     * 判断指定 TileEntity 是否位于通用供应节点黑名单中。
     * 黑名单中的实体只能由覆写了 {@code isBlacklisted} 的专用节点建立连接，普通供应节点无法连接。
     * <p>
     * Returns whether the given TileEntity is on the supply-node blacklist.
     * Blacklisted entities can only be connected by specialized nodes that override {@code isBlacklisted};
     * generic supply nodes cannot connect to them.
     *
     * @param tileEntity 目标实体 / the tile entity to check
     * @return 是否在供应黑名单中 / {@code true} if on the supply blacklist
     */
    public static boolean isSupplyBlacklisted(@Nonnull TileEntity tileEntity) {
    ...
    /**
     * 判断指定 TileEntity 是否为受能量处理器管理的能源容器。
     * <p>
     * Returns whether the given TileEntity is an energy container handled by a registered energy manager.
     *
     * @param tileEntity 目标实体 / the tile entity to check
     * @return 是否为能源容器 / {@code true} if the tile entity is an energy container
     */
    public static boolean isEnergyTileEntity(@Nonnull TileEntity tileEntity) {
        return RegistryEnergyHandler.isEnergyTileEntity(tileEntity);
    }

    /**
     * 获取适用于指定 TileEntity 的能量处理器管理器。
     * <p>
     * Returns the energy handler manager compatible with the given TileEntity,
     * or {@code null} if no registered manager applies.
     *
     * @param tileEntity 目标实体 / the tile entity to query
     * @return 匹配的能量管理器，若无匹配则为 {@code null} / a matching manager, or {@code null} if none applies
     */
    @Nullable
    public static IEnergyHandlerManager getEnergyManager(@Nonnull TileEntity tileEntity) {
        return RegistryEnergyHandler.getEnergyManager(tileEntity);
    }
