...
public final class NodeNetworkRendering implements Packet<NodeNetworkRendering> {
    ...
    public static final Type<NodeNetworkRendering> TYPE = new Type<>(
        Identifier.parse(CirculationFlowNetworks.MOD_ID + ":node_network_rendering")
    );
    ...
    private static void writeLinks(RegistryFriendlyByteBuf buf, IntSupplier writer) {
    ...
    private static long[] readLongPairs(RegistryFriendlyByteBuf buf) {
    ...
    public void handle(@NonNull NodeNetworkRendering message, @NonNull IPayloadContext context) {
        context.enqueueWork(() -> {
